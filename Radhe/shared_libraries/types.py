...
class AttractionEvent(BaseModel):
    """An Attraction."""
    event_type: str = Field(description="Type of event, typically 'visit'")
...
class FlightEvent(BaseModel):
    ...
    event_type: str = Field(description="Type of event, typically 'flight'")
...
class HotelEvent(BaseModel):
    ...
    event_type: str = Field(description="Type of event, typically 'hotel'")
    description: str = Field(description="A name, title or a description of the hotel")
    address: str = Field(description="Full address of the attraction")
    check_in_time: str = Field(description="Time in HH:MM format, e.g. 16:00")
    check_out_time: str = Field(description="Time in HH:MM format, e.g. 15:30")
    room_selection: str = Field(description="Selected room type")
    booking_required: bool = Field(description="Whether booking is required")
...
class ItineraryDay(BaseModel):
    ...
    events: list[Union[FlightEvent, HotelEvent, AttractionEvent]] = Field(
        description="The list of events for the day"
    )
...
class UserProfile(BaseModel):
    ...
    allergies: list[str] = Field(
        description="A list of food allegies to avoid"
    )
    diet_preference: list[str] = Field(
        description="Vegtarian, Vegan... etc."
    )
